...
function delByCid() {
   ...
   if(dels.length<=0) {
     ...
     alert("一次最多只能选择一个推送对象!"); 
   }else {
     var url ="<%=basePath%>servlet/RegisterServlet?dels="+dels+"&method=sendMail";
     window.location=url;
   }
 }
</script>		
...
.table td{border:1px #d2d2d2 solid;padding:5px 4px;height:42px;width:30px}
.table td a{color:#668B8B;margin:0 5px;cursor:pointer;}
  ...
  %>
    ...
    <h1 class="h1">自动推送推荐岗位 &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="condition" class="condition" style="color:#999999;font-family: 'Open Sans', sans-serif;" placeholder="请输入学生姓名/学号" name="condition"> <button type="button" class="button gray" onclick="findClerkByCondition()">查询</button>
	</h1>  
	<table border='1' class="table"  bordercolor="#CAE1FF" cellspacing="0" cellpadding="0">
		<tr>
			...
			<th class='onecenter' style="color: #7A7A7A">选择</th>
		</tr>
		...
		<button type="button" class="button gray" onclick="delByCid()">自动推送岗位</button>
		</td>
		</tr>
	</table>
	<br/>
 ...
 <a href="<%=basePath %>student/sendMail.jsp?pageIndex=1" style="color:#575757;font-family: 'Open Sans', sans-serif;">首页</a>
   
 <%if(p.getPageIndex()>1) {%>
 
 <a href="<%=basePath %>student/sendMail.jsp?pageIndex=<%=pageIndex-1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">上一页</a>
 
 <%}  if(p.getPageIndex()<p.getCount()) {%>
 
 <a href="<%=basePath %>student/sendMail.jsp?pageIndex=<%=pageIndex+1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">下一页</a>
 
 <% }%>
  
 <a href="<%=basePath %>student/sendMail.jsp?pageIndex=<%=p.getCount() %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">末页</a> 
   共<%=p.getCount() %>页</span>
   <%} %>	
  <!-- 分页 --> 

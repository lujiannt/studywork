...
<script type="text/javascript">
	function findSalaryByCondition() {
	   ...
	   var url = "<%=basePath%>salary/findSalary.jsp?method=findSalaryByCondition&condition="+condition;
	   window.location =url;
	   }else {
	   alert("请输入查询条件!");
	   }
	...
	function findSalaryBySel() {
	  ...
	  if(orderCondition=="salaryup") {
	   ...
	   var url = "<%=basePath%>salary/findSalary.jsp?method=findOrderSalaryByCondition&condition="+condition;
	   window.location =url;
...
%>
 ...
 <a href="<%=basePath %>salary/findSalary.jsp?method=<%=method %>&condition=${condition}&pageIndex=1" style="color:#575757;font-family: 'Open Sans', sans-serif;">首页</a>
   
 <%if(p.getPageIndex()>1) {%>
 
 <a href="<%=basePath %>salary/findSalary.jsp?method=<%=method %>&condition=${condition}&pageIndex=<%=pageIndex-1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">上一页</a>
 
 <%}  if(p.getPageIndex()<p.getCount()) {%>
 
 <a href="<%=basePath %>salary/findSalary.jsp?method=<%=method %>&condition=${condition}&pageIndex=<%=pageIndex+1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">下一页</a>
 
 <% }%>
  
 <a href="<%=basePath %>salary/findSalary.jsp?method=<%=method %>&condition=${condition}&pageIndex=<%=p.getCount() %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">末页</a> 
   共<%=p.getCount() %>页</span>
   <%} %>	
  <!-- 分页 --> 

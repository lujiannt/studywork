...
.table td{border:1px #d2d2d2 solid;padding:8px 6px;height:50px;width:30px}
  ...
  %>
	...
	<table  class="table" border='1' bordercolor="#CAE1FF" cellspacing="0" cellpadding="0">
		<tr>
			...
			<th class='onecenter' style="color: #7A7A7A">银行卡号</th>
		</tr>
		...
		<tr>
		  <td  align="center" colspan="12" class='onecenter'
 ...
 <a href="<%=basePath %>clerk/findClerk.jsp?pageIndex=1" style="color:#575757;font-family: 'Open Sans', sans-serif;">首页</a>
   
 <%if(p.getPageIndex()>1) {%>
 
 <a href="<%=basePath %>clerk/findClerk.jsp?pageIndex=<%=pageIndex-1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">上一页</a>
 
 <%}  if(p.getPageIndex()<p.getCount()) {%>
 
 <a href="<%=basePath %>clerk/findClerk.jsp?pageIndex=<%=pageIndex+1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">下一页</a>
 
 <% }%>
  
 <a href="<%=basePath %>clerk/findClerk.jsp?pageIndex=<%=p.getCount() %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">末页</a> 
   共<%=p.getCount() %>页</span>
   <%} %>	
  <!-- 分页 --> 

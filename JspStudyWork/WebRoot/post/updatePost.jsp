...
<script type="text/javascript">
	function findPostByCondition() {
	   var condition =document.getElementById("condition").value;
	   if(condition!=null&&condition!="") {
	   var url = "<%=basePath%>post/updatePost.jsp?method=findPostByCondition&condition="+condition;
	   window.location =url;
	   }else {
	   alert("请输入查询条件!");
	   }
...
var mes = confirm("确定将此记录删除?");
   ...
   if(dels.length<=0) {
     ...
     var url ="<%=basePath%>servlet/PostServlet?dels="+dels+"&method=delPost";
     window.location=url;
   }
 }
 }	
...
%>
    ...
    <h1 class="h1">岗位编辑 &nbsp;&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="condition" class="condition" style="color:#999999;font-family: 'Open Sans', sans-serif;" placeholder="请输入岗位编号或名称" name="condition"> <button type="button" class="button gray" onclick="findPostByCondition()">查询</button>
	</h1>  
	<table border='1' class="table" bordercolor="#CAE1FF" cellspacing="0" cellpadding="0">
		...
		<c:forEach items="${list}" var="post">
			<tr>
			    ...
			    <td class='onecenter'><a href="<%=basePath %>post/updatePostDetail.jsp?pid=${post.pid}&pname=${post.pname}&p_oid=${post.p_oid}&pleaderphone=${post.pleaderphone}&pleadername=${post.pleadername}&pcount=${post.pcount}&p_oname=${post.p_oname}&p_dname=${post.p_dname}&pcount2=${post.pcount2}&kind=${post.kind}" >修改</a></td>
				<td class='onecenter'><input type="checkbox" name="dels" value="${post.pid}"></td>
			</tr>
		</c:forEach>
		<!-- 没有找到的情况下 -->
 ...
 <a href="<%=basePath %>post/updatePost.jsp?pageIndex=1" style="color:#575757;font-family: 'Open Sans', sans-serif;">首页</a>
   
 <%if(p.getPageIndex()>1) {%>
 
 <a href="<%=basePath %>post/updatePost.jsp?pageIndex=<%=pageIndex-1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">上一页</a>
 
 <%}  if(p.getPageIndex()<p.getCount()) {%>
 
 <a href="<%=basePath %>post/updatePost.jsp?pageIndex=<%=pageIndex+1 %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">下一页</a>
 
 <% }%>
  
 <a href="<%=basePath %>post/updatePost.jsp?pageIndex=<%=p.getCount() %>" style="color:#575757;font-family: 'Open Sans', sans-serif;">末页</a> 
   共<%=p.getCount() %>页</span>
   <%} %>	
  <!-- 分页 --> 
